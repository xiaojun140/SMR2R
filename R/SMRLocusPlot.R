...
SMRLocusPlot = function(data=SMRData, probeNEARBY=NULL,smr_thresh=NULL, smr_thresh_plot=NULL, heidi_thresh=NULL, plotWindow=NULL,pointsize=20,max_anno_probe=16,anno_selfdef=TRUE)
{
  ...
  generow = SMR2R::GeneRowNum(glist);
  ...
  for( k in 1 : num_row ) {
    ...
    xcenter = TeachingDemos::spread.labs(xcenter, mindiff=0.01, maxiter=1000, min = xmin, max = xmax)
  ...
  if(anno_selfdef)
  {
    ...
    xcenter = TeachingDemos::spread.labs(xcenter[1:nprobeINFO], mindiff=0.08, maxiter=1000, min = xmin, max = xmax-1)
  }
  ...
  for( k in 1 : nprobeINFO)  {
    ...
    # plot p value of bxy
    SMR2R::plot_probe(probeINFO, k, colplot, xmin, xmax, yaxis.min, yaxis.max,pchbuf,hitflag)
    # annotate the probes
    if(k<=max_anno_probe)
    {
      ypos = 1.02*yMAX
